...
plot4 <- function() {
        ...
        plot(strptime(paste(pc$Date, pc$Time,sep = ":"),format='%Y-%m-%d:%H:%M:%S'),as.numeric(pc$Voltage), type = "l")
        ...
        plot(strptime(paste(pc$Date, pc$Time,sep = ":"),format='%Y-%m-%d:%H:%M:%S'),as.numeric(pc$Global_reactive_power), type = "l")
